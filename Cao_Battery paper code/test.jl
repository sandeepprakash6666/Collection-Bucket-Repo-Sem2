...
##*Input Profile: bioreactor (As ODE) 
  #!Not matching - could be error propogating in forward simulation
  ...
  function step_input(t)
    ...
    # u_step = [0.043218149808586404 0.09321814249089909 0.14321810061527981 0.19321611046929404 0.19094903219562648 0.18630707923527773 0.18900514474434263 0.20031173869143587 0.2176199324313479 0.23692608547391864 0.2560988580394867 0.283627597163917 0.33362749533148794 0.3836274904148855 0.4336274942375861 0.4836274985220684 0.45998824410278555 0.3174001639074914 0.2965632303630175 0.2989649803955313 0.3001122498881744 0.30015803414129366 0.3000936658016556 0.30006273123797705 0.3000462764544888 0.30003441778946877 0.300025428017568 0.3000187077965616 0.3000137002471859 0.3000103083659555]
    u_step = [0.2916572110275656 0.3416572201748369 0.3916572288927861 0.33174142317075545]
    NFE = size(u_step)[2]
  ...
  sol = solve(prob,  alg )
  plot(sol, ylim = [0.0, 2.0], title = "Sim as ODE : $alg "  )



  # t_plot = collect(0:1:30.0)
  # u_step = [0.043218149808586404 0.09321814249089909 0.14321810061527981 0.19321611046929404 0.19094903219562648 0.18630707923527773 0.18900514474434263 0.20031173869143587 0.2176199324313479 0.23692608547391864 0.2560988580394867 0.283627597163917 0.33362749533148794 0.3836274904148855 0.4336274942375861 0.4836274985220684 0.45998824410278555 0.3174001639074914 0.2965632303630175 0.2989649803955313 0.3001122498881744 0.30015803414129366 0.3000936658016556 0.30006273123797705 0.3000462764544888 0.30003441778946877 0.300025428017568 0.3000187077965616 0.3000137002471859 0.3000103083659555]
  # plot(t_plot[1:end-1], u_step[1,:], linetype = :steppost)
...
##*Input Profile: bioreactor (As DAE) 
  ...
  plot(sol, ylim = [0.0, 2.0], title = "Sim as DAE")
...
##*Naive approach : Calling integrator in for loop
  ...
  #           ]
    ...
    p1 = plot(t_list, [x_list[i][1] for i in 1:N_list], ylim = (0.0, 2.0), title = "Naive")
    p1 = plot!(t_list, [x_list[i][2] for i in 1:N_list])
