...
function OptimalControl(u0, t_start, soc_min, soc_max)
    ...
    @constraint(
        ...
        buy_from_grid[max(1, Int(ceil(TIME_FR[i] / 3600)))] - waste[i]
    )
    ...
    Un_guess =
        9.99877 - 9.99961 * theta_n_guess .^ 0.5 - 9.98836 * theta_n_guess +
    ...
    Up_guess =
        7.49983 - 13.7758 * theta_p_guess .^ 0.5 + 21.7683 * theta_p_guess -
    ...
    if soc_min != soc_max
        @variable(
            m,
            capacity_remain * (back + soc_min_stop) * csnmax <=
            csn_avg[t in 2:Nt] <=
            capacity_remain * (soc_max_stop - back) * csnmax,
            start = csn_avg0
    ...
    @constraint(
        m,
        csp_avg_Euler[t = 2:Nt-1],
        (csp_avg[t+1] - csp_avg[t]) / dt ==
        -15 * Dp / Rpp / Rpp * (csp_avg[t+1] - csp_s[t+1])
    )
    @constraint(
        m,
        csp_s_Euler[t = 2:Nt],
        0 == 1e3 * (5 * (csp_s[t] - csp_avg[t]) + Rpp * it[t] / F / Dp / ap / lp)
    )
    t = 1
    @constraint(
        m,
        (csp_avg[t+1] - csp_avg0) / dt ==
        -15 * Dp / Rpp / Rpp * (csp_avg[t+1] - csp_s[t+1])
    )
    ...
    if soc_min != soc_max
        @constraint(
            m,
            csn_avg_Euler[t = 2:Nt-1],
            (csn_avg[t+1] - csn_avg[t]) / dt ==
            -15 * Dn / Rpn / Rpn * (csn_avg[t+1] - csn_s[t+1])
        )
        @constraint(
            m,
            csn_s_Euler[t = 2:Nt],
            0 == 1e3 * (5 * (csn_s[t] - csn_avg[t]) - Rpn * iint[t] / F / Dn / an / lnn)
        )

        t = 1
        @constraint(
            m,
            (csn_avg[t+1] - csn_avg0) / dt ==
            -15 * Dn / Rpn / Rpn * (csn_avg[t+1] - csn_s[t+1])
        )
        @constraint(
            m,
            0 == 1e3 * (5 * (csn_s[t] - csn_avg0) - Rpn * iint[t] / F / Dn / an / lnn)
        )
    else
        @constraint(
            m,
            csn_avg_Euler[t = 2:Nt-2],
            (csn_avg[t+1] - csn_avg[t]) / dt ==
            -15 * Dn / Rpn / Rpn * (csn_avg[t+1] - csn_s[t+1])
        )
        @constraint(
            m,
            csn_s_Euler[t = 2:Nt-1],
            0 == 1e3 * (5 * (csn_s[t] - csn_avg[t]) - Rpn * iint[t] / F / Dn / an / lnn)
        )
        ...
        @constraint(
            m,
            (soc_min * csnmax * capacity_remain - csn_avg[t]) / dt ==
            -15 * Dn / Rpn / Rpn * (soc_min * csnmax * capacity_remain - csn_s[t+1])
        )
        t = Nt
        @constraint(
            m,
            0 ==
            1e3 * (
                5 * (csn_s[t] - soc_min * csnmax * capacity_remain) -
                Rpn * iint[t] / F / Dn / an / lnn
            )
        )
    end
    ...
    @NLconstraint(
        ...
        sinh(
            0.5 * F / R / T *
            (phi_n[t] - Un[t] + delta_sei0 / Kappa_sei * it[t] / an / lnn),
        )
    )
    @NLconstraint(
        m,
        ...
        sinh(
            0.5 * F / R / T *
            (phi_n[t] - Un[t] + delta_sei[t] / Kappa_sei * it[t] / an / lnn),
        )
    )
    ...
    @NLconstraint(
        ...
        Up_def[t = TIMEG],
        Up[t] ==
        7.49983 - 13.7758 * theta_p[t]^0.5 + 21.7683 * theta_p[t] -
        12.6985 * theta_p[t]^1.5 + 0.0174967 / theta_p[t] - 0.41649 * theta_p[t]^(-0.5) -
        0.0161404 * exp(100 * theta_p[t] - 97.1069) +
        0.363031 * tanh(5.89493 * theta_p[t] - 4.21921)
    )
    @NLconstraint(
        m,
        Un_def[t = TIMEG],
        Un[t] ==
        9.99877 - 9.99961 * theta_n[t]^0.5 - 9.98836 * theta_n[t] +
        8.2024 * theta_n[t]^1.5 +
        0.23584 / theta_n[t] - 2.03569 * theta_n[t]^(-0.5) -
        1.47266 * exp(-1.14872 * theta_n[t] + 2.13185) -
        9.9989 * tanh(0.60345 * theta_n[t] - 1.58171)
    )
    ...
    @constraint(
        m,
        [t in mTIMEG],
        power[max(1, Int(ceil(RealTime[t] / dt_FR)))] == pot[t] * it[t]
    )
    ...
    @variable(
        ...
        start =
            ...
            exp(
                -1 * F / R / T *
                (Un_guess - Urefs + delta_sei0 / Kappa_sei * it0[t] / an / lnn),
            )
    )
    ...
    @NLconstraint(
        ...
        ksei *
        (exp(
            -1 * F / R / T * (phi_n[t] - Urefs + delta_sei0 / Kappa_sei * it[t] / an / lnn),
        ))
    )
    @NLconstraint(
        m,
        ...
        (exp(
            -1 * F / R / T *
            (phi_n[t] - Urefs + delta_sei[t] / Kappa_sei * it[t] / an / lnn),
        ))
    )
    ...
    @constraint(
        m,
        (delta_sei[t+1] - delta_sei0) / dt == isei[t+1] * M_sei / F / rho_sei / an / lnn
    )
    @constraint(
        m,
        delta_sei_Euler[t = 2:Nt-1],
        (delta_sei[t+1] - delta_sei[t]) / dt == isei[t+1] * M_sei / F / rho_sei / an / lnn
    )
    ...
    println(
        "penalty   ",
        expectedrevenue / (1 - soc_retire) * (getvalue(cf[Nt]) - cf0) / Qmax,
    )
